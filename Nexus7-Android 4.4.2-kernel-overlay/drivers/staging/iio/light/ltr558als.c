...
#define LTR558_POLL_INTERVAL_MS 20
...
static void ltr558_rawdata_poll(struct input_polled_dev *ipdev)
{
    // TODO check that private is set correctly
    struct ltr558_chip *chip= (struct ltr558_chip *) ipdev->private;

	chip->als_reading = ltr558_als_read(chip->client);
	chip->prox_reading = ltr558_ps_read(chip->client);
 
    input_report_abs(ipdev->input, ABS_X, chip->als_reading);
    input_report_abs(ipdev->input, ABS_Y, chip->prox_reading);
    input_sync(ipdev->input);
}

static int ltr558_input_init(struct ltr558_chip *chip)
{
    int result =-1;
    struct input_polled_dev *ipdev;
    ...
    if (!ipdev) {
        dev_dbg(&chip->client->dev, "error creating input device\n");
        return -ENOMEM;
    }

    ipdev->poll = ltr558_rawdata_poll;
    ipdev->poll_interval = LTR558_POLL_INTERVAL_MS;
    ipdev->private = chip;

        ipdev->input->name = "ltr558";
        ipdev->input->id.bustype = BUS_HOST;

        set_bit(EV_ABS, ipdev->input->evbit);

    input_set_abs_params(ipdev->input, ABS_X, -2048, 2047, 0, 0);
    input_set_abs_params(ipdev->input, ABS_Y, -2048, 2047, 0, 0);

    result = input_register_polled_device(ipdev);
    if (result) {
		...
		const struct i2c_device_id *id)
{
    ...
    ret= ltr558_input_init(chip);
    if (ret !=0 ) {
        pr_err("%s: cound not register as polled input device (%d).\n",
            __func__, ret);
...
static const struct dev_pm_ops ltr558_dev_pm_ops = {
