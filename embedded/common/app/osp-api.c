...
static void OnCalibratedSensor(enum SensorType_t sensorType,
    const NTTIME time,
    const int32_t calibratedData[NUM_TRIAXIS_SENSOR_AXES])
{
...
static int16_t DeactivateResultSensors(const struct SensorId_t *sensorId)
{
    ...
    for (i = 0; i < _ResultResourceMap[index].SensorCount; i++) { // for each of our sensors
        ...
        for (k = 0; k < MAX_RESULT_DESCRIPTORS; k++) {
            if ((_ResultTable[k].pResDesc != NULL) &&
                (_ResultTable[k].pResDesc->sensorId.sensorType != sensorId->sensorType)) { // search active results (but not ours)
                ...
                for (l = 0; l < _ResultResourceMap[j].SensorCount; l++) { // for each sensor in this active result
                    if ((_ResultResourceMap[j].Sensors[l].sensorType ==
                            _ResultResourceMap[index].Sensors[i].sensorType) &&
                        (_ResultResourceMap[j].Sensors[l].sensorType ==
                            _ResultResourceMap[index].Sensors[i].sensorSubType)) {
