...
#define ACCEL_UNSCALING_FACTOR          TOFIX_PRECISE(1.0f / (ACCEL_SENSITIVITY_4G * M_SI_EARTH_GRAVITY))  // LSB / earth grav
#define ACCEL_RANGE_MAX                 TOFIX_EXTENDED(4.0f * M_SI_EARTH_GRAVITY)
#define ACCEL_NOISE                     TOFIX_PRECISE(1.0f / M_SI_EARTH_GRAVITY)
#define ACCEL_INPUT_RATES               TOFIX_EXTENDED(12.5f), TOFIX_EXTENDED(25.0f), TOFIX_EXTENDED(50.0f), \
    TOFIX_EXTENDED(100.0f)
#define ACCEL_OUTPUT_RATES              TOFIX_EXTENDED(50.0f), TOFIX_EXTENDED(0.0f), TOFIX_EXTENDED(0.0f), \
    TOFIX_EXTENDED(0.0f)

#define MAG_SENSITIVITY_2_5G_XY         1.4925f  //mg/LSB
#define MAG_SCALING_XY                  TOFIX_EXTENDED(MAG_SENSITIVITY_2_5G_XY * 0.1f)  //uT/LSB
#define MAG_UNSCALING_XY                TOFIX_EXTENDED(1.0f / (MAG_SENSITIVITY_2_5G_XY * 0.1f))  //LSB / ut
#define MAG_SENSITIVITY_2_5G_Z          1.6666f  //mg/LSB
#define MAG_SCALING_Z                   TOFIX_EXTENDED(MAG_SENSITIVITY_2_5G_Z * 0.1f)  //uT/LSB
#define MAG_UNSCALING_Z                 TOFIX_EXTENDED(1.0f / (MAG_SENSITIVITY_2_5G_Z * 0.1f))  //LSB / ut
#define MAG_RANGE_MAX                   (2.5f * 100.0f) //µT
#define MAG_NOISE                       TOFIX_PRECISE(0.2f)
#define MAG_INPUT_RATES                 TOFIX_EXTENDED(12.5f), TOFIX_EXTENDED(25.0f), TOFIX_EXTENDED(50.0f), \
    TOFIX_EXTENDED(100.0f)
#define MAG_OUTPUT_RATES                TOFIX_EXTENDED(50.0f), TOFIX_EXTENDED(0.0f), TOFIX_EXTENDED(0.0f), \
    TOFIX_EXTENDED(0.0f)

#define GYRO_SENSITIVITY                0.07f      //70mdps/digit
#define GYRO_SCALING_FACTOR             TOFIX_PRECISE(GYRO_SENSITIVITY * 0.0175f)  //rad/sec/lsb
#define GYRO_UNSCALING_1_FACTOR         TOFIX_PRECISE(1.0f / 0.0175f)  //  lsb / rad
#define GYRO_UNSCALING_2_FACTOR         TOFIX_PRECISE(1.0f / GYRO_SENSITIVITY)  //  lsb / deg

#define GYRO_RANGE_MAX                  (2000.0f * 0.017453f) //in rad/sec
#define GYRO_NOISE                      TOFIX_PRECISE(0.006f)
#define GYRO_INPUT_RATES                TOFIX_EXTENDED(12.5f), TOFIX_EXTENDED(25.0f), TOFIX_EXTENDED(50.0f), \
    TOFIX_EXTENDED(100.0f)
#define GYRO_OUTPUT_RATES               TOFIX_EXTENDED(50.0f), TOFIX_EXTENDED(0.0f), TOFIX_EXTENDED(0.0f), \
    TOFIX_EXTENDED(0.0f)

#define XYZ_FROM_ORIGIN                 TOFIX_PRECISE(0.0f)

#define STEP_COUNT_OUTPUT_RATE          TOFIX_EXTENDED(0.0f), TOFIX_EXTENDED(0.0f), TOFIX_EXTENDED(0.0f), \
    TOFIX_EXTENDED(0.0f)
#define SIG_MOTION_OUTPUT_RATE          TOFIX_EXTENDED(0.0f), TOFIX_EXTENDED(0.0f), TOFIX_EXTENDED(0.0f), \
    TOFIX_EXTENDED(0.0f)
...
static void stepCounterOutputCallback(
    OutputSensorHandle_t outputHandle,
    Android_StepCounterOutputData_t*pOutput);

static void stepDetectorOutputCallback(
    OutputSensorHandle_t outputHandle,
    Android_StepDetectorOutputData_t*pOutput);

static void sigMotionOutputCallback(
    OutputSensorHandle_t outputHandle,
    Android_SignificantMotionOutputData_t*pOutput);

static void UnCalAccelDataResultCallback(
    OutputSensorHandle_t outputHandle,
    Android_UncalibratedAccelOutputData_t*pOutput);

static void CalAccelDataResultCallback(
    OutputSensorHandle_t outputHandle,
    Android_CalibratedAccelOutputData_t*pOutput);

static void UnCalMagDataResultCallback(
    OutputSensorHandle_t outputHandle,
    Android_UncalibratedMagOutputData_t*pOutput);

static void CalMagDataResultCallback(
    OutputSensorHandle_t outputHandle,
    Android_CalibratedMagOutputData_t*pOutput);

static void UnCalGyroDataResultCallback(
    OutputSensorHandle_t outputHandle,
    Android_UncalibratedGyroOutputData_t*pOutput);

static void CalGyroDataResultCallback(
    OutputSensorHandle_t outputHandle,
...
static ResultOptionalData_t accelUnscaleFactor = {
    QFIXEDPOINTPRECISE,
    {ACCEL_UNSCALING_FACTOR, ACCEL_UNSCALING_FACTOR, ACCEL_UNSCALING_FACTOR }
}; /* index per Axis */
...
static ResultOptionalData_t magUnscaleFactor = {
    QFIXEDPOINTEXTENDED,
    { MAG_UNSCALING_XY, MAG_UNSCALING_XY, MAG_UNSCALING_Z }
}; /* index per Axis */
...
static ResultOptionalData_t gyroUnscaleFactor = {
    QFIXEDPOINTPRECISE,
    { GYRO_UNSCALING_1_FACTOR, GYRO_UNSCALING_2_FACTOR, 0 }
}; /* index NOT per Axis - see computation */
...
#ifndef multiplyAndRound
/****************************************************************************************************
 * @fn      multiplyAndRound
 *          Helper routine that can be replaced with optimized implementation
 *
 ***************************************************************************************************/
    ...
    Android_UncalibratedAccelOutputData_t*pOutput)
{
    ...
    hostBuffer.data.uncalibratedSensordata.Data[0] = multiplyAndRound(
        pOutput->X,
        accelUnscaleFactor.unscale.ntPreciseUnscaleFactor[0],
        QFIXEDPOINTPRECISE);
    hostBuffer.data.uncalibratedSensordata.Data[1] = multiplyAndRound(
        pOutput->Y,
        accelUnscaleFactor.unscale.ntPreciseUnscaleFactor[1],
        QFIXEDPOINTPRECISE);
    hostBuffer.data.uncalibratedSensordata.Data[2] = multiplyAndRound(
        pOutput->Z,
        accelUnscaleFactor.unscale.ntPreciseUnscaleFactor[2],
        QFIXEDPOINTPRECISE);
    ...
    post_on_boardcast_buffer(
        (uint8_t*)&hostBuffer,
        sizeof(hostBuffer.data.uncalibratedSensordata),
        &hostBuffer.data.uncalibratedSensordata.timeStamp);

    if (g_logging & 0x40) { //Uncalibrated data, in Android conventions
        Print_LIPS("RA,%.6f,%.6f,%.6f,%.6f",
            TOFLT_TIME(pOutput->TimeStamp),
            TOFLT_PRECISE(pOutput->X),
            TOFLT_PRECISE(pOutput->Y),
            TOFLT_PRECISE(pOutput->Z));
    }
}
    ...
    Android_CalibratedAccelOutputData_t*pOutput)
{
    ...
    post_on_boardcast_buffer(
        (uint8_t*)&hostBuffer,
        sizeof(hostBuffer.data.sensorData),
        &hostBuffer.data.sensorData.timeStamp);

    if (g_logging & 0x40) { //Uncalibrated data, in Android conventions
        Print_LIPS("A,%.6f,%.6f,%.6f,%.6f",
            TOFLT_TIME(pOutput->TimeStamp),
            TOFLT_PRECISE(pOutput->X),
            TOFLT_PRECISE(pOutput->Y),
            TOFLT_PRECISE(pOutput->Z));
    ...
    Android_UncalibratedMagOutputData_t*pOutput)
{
    ...
    hostBuffer.data.uncalibratedSensordata.Data[0] = multiplyAndRound(
        pOutput->X,
        magUnscaleFactor.unscale.ntExtendedUnscaleFactor[0],
        QFIXEDPOINTEXTENDED);

    hostBuffer.data.uncalibratedSensordata.Data[1] = multiplyAndRound(
        pOutput->Y,
        magUnscaleFactor.unscale.ntExtendedUnscaleFactor[1],
        QFIXEDPOINTEXTENDED);

    hostBuffer.data.uncalibratedSensordata.Data[2] = multiplyAndRound(
        pOutput->Z,
        magUnscaleFactor.unscale.ntExtendedUnscaleFactor[2],
        QFIXEDPOINTEXTENDED);

    hostBuffer.data.uncalibratedSensordata.Offset[0] = multiplyAndRound(
        pOutput->X_hardIron_offset,
        magUnscaleFactor.unscale.ntExtendedUnscaleFactor[0],
        QFIXEDPOINTEXTENDED);

    hostBuffer.data.uncalibratedSensordata.Offset[1] = multiplyAndRound(
        pOutput->Y_hardIron_offset,
        magUnscaleFactor.unscale.ntExtendedUnscaleFactor[1],
        QFIXEDPOINTEXTENDED);

    hostBuffer.data.uncalibratedSensordata.Offset[2] = multiplyAndRound(
        pOutput->Z_hardIron_offset,
        magUnscaleFactor.unscale.ntExtendedUnscaleFactor[2],
        QFIXEDPOINTEXTENDED);

    post_on_boardcast_buffer((uint8_t*)&hostBuffer,
        sizeof(hostBuffer.data.uncalibratedSensordata),
        &hostBuffer.data.uncalibratedSensordata.timeStamp);
    ...
    if (g_logging & 0x40) { //Uncalibrated data, in Android conventions
        Print_LIPS(
            "RM,%.6f,%.6f,%.6f,%.6f",
            TOFLT_TIME(pOutput->TimeStamp),
            TOFLT_EXTENDED(pOutput->X),
            TOFLT_EXTENDED(pOutput->Y),
            TOFLT_EXTENDED(pOutput->Z));
    }
}
    ...
    Android_UncalibratedGyroOutputData_t*pOutput)
{
    ...
    hostBuffer.data.uncalibratedSensordata.Offset[0] = multiplyAndRound(
        ((int64_t)pOutput->X_drift_offset *
            (int64_t)gyroUnscaleFactor.unscale.ntPreciseUnscaleFactor[0]) >> QFIXEDPOINTPRECISE,
        gyroUnscaleFactor.unscale.ntPreciseUnscaleFactor[1],
        QFIXEDPOINTPRECISE);

    hostBuffer.data.uncalibratedSensordata.Offset[1] = multiplyAndRound(
        ((int64_t)pOutput->Y_drift_offset *
            (int64_t)gyroUnscaleFactor.unscale.ntPreciseUnscaleFactor[0]) >> QFIXEDPOINTPRECISE,
        gyroUnscaleFactor.unscale.ntPreciseUnscaleFactor[1],
        QFIXEDPOINTPRECISE);

    hostBuffer.data.uncalibratedSensordata.Offset[2] = multiplyAndRound(
        ((int64_t)pOutput->Z_drift_offset *
            (int64_t)gyroUnscaleFactor.unscale.ntPreciseUnscaleFactor[0]) >> QFIXEDPOINTPRECISE,
        gyroUnscaleFactor.unscale.ntPreciseUnscaleFactor[1],
        QFIXEDPOINTPRECISE);
    ...
    Android_StepCounterOutputData_t*pOutput)
{
    ...
    post_on_boardcast_buffer(
        (uint8_t*)&hostBuffer,
        sizeof(hostBuffer.data.stepCounterData),
        &hostBuffer.data.stepCounterData.timeStamp);
    ...
    Android_StepDetectorOutputData_t*pOutput)
{
    ...
    post_on_boardcast_buffer((uint8_t*)&hostBuffer,
        sizeof(hostBuffer.data.stepDetectorData),
        &hostBuffer.data.stepDetectorData.timeStamp);
    ...
    Android_SignificantMotionOutputData_t*pOutput)
{
    ...
    post_on_boardcast_buffer((uint8_t*)&hostBuffer,
        sizeof(hostBuffer.data.significantMotionData),
        &hostBuffer.data.significantMotionData.timeStamp);
                ...
                case SENSOR_ACCELEROMETER_UNCALIBRATED:
                    if (rcvMsg->msg.msgSensorEnable.enabled) {
                        if (_unCalAccelHandle == NULL) {
                            ...
                            ASF_assert_msg(
                                OSP_STATUS_OK == OSP_Status,
                                "SensorManager: OSP_SubscribeResult (SENSOR_ACCELEROMETER_UNCALIBRATED) Failed");
                        }
                    } else {
                        if (_unCalAccelHandle != NULL) {
                ...
                case SENSOR_ACCELEROMETER_CALIBRATED:
                    if (rcvMsg->msg.msgSensorEnable.enabled) {
                        if (_calAccelHandle == NULL) {
                            ...
                            ASF_assert_msg(
                                OSP_STATUS_OK == OSP_Status,
                                "SensorManager: OSP_SubscribeResult (SENSOR_ACCELEROMETER_CALIBRATED) Failed");
                        }
                    } else {
                        if (_calAccelHandle != NULL) {
                ...
                case SENSOR_MAGNETIC_FIELD_UNCALIBRATED:
                    if (rcvMsg->msg.msgSensorEnable.enabled) {
                        if (_unCalMagHandle == NULL) {
                            ...
                            ASF_assert_msg(
                                OSP_STATUS_OK == OSP_Status,
                                "SensorManager: OSP_SubscribeResult (SENSOR_MAGNETIC_FIELD_UNCALIBRATED) Failed");
                        }
                    } else {
                        if (_unCalMagHandle != NULL) {
                ...
                case SENSOR_MAGNETIC_FIELD_CALIBRATED:
                    if (rcvMsg->msg.msgSensorEnable.enabled) {
                        if (_calMagHandle == NULL) {
                            ...
                            ASF_assert_msg(
                                OSP_STATUS_OK == OSP_Status,
                                "SensorManager: OSP_SubscribeResult (SENSOR_MAGNETIC_FIELD_CALIBRATED) Failed");
                        }
                    } else {
                        if (_calMagHandle != NULL) {
                ...
                case SENSOR_GYROSCOPE_UNCALIBRATED:
                    if (rcvMsg->msg.msgSensorEnable.enabled) {
                        if (_unCalGyroHandle == NULL) {
                            ...
                            ASF_assert_msg(
                                OSP_STATUS_OK == OSP_Status,
                                "SensorManager: OSP_SubscribeResult (SENSOR_GYROSCOPE_UNCALIBRATED) Failed");
                        }
                    } else {
                        if (_unCalGyroHandle != NULL) {
                ...
                case SENSOR_GYROSCOPE_CALIBRATED:
                    if (rcvMsg->msg.msgSensorEnable.enabled) {
                        if (_calGyroHandle == NULL) {
                            ...
                            ASF_assert_msg(
                                OSP_STATUS_OK == OSP_Status,
                                "SensorManager: OSP_SubscribeResult (SENSOR_GYROSCOPE_CALIBRATED) Failed");
                        }
                    } else {
                        if (_calGyroHandle != NULL) {
                ...
                case SENSOR_STEP_COUNTER:
                    if (rcvMsg->msg.msgSensorEnable.enabled) {
                        if (_stepCounterHandle == NULL) {
                            ...
                            ASF_assert_msg(
                                OSP_STATUS_OK == OSP_Status,
                                "SensorManager: OSP_SubscribeResult (SENSOR_STEP_COUNTER) Failed");
                        }
                    } else {
                        if (_stepCounterHandle != NULL) {
                ...
                case SENSOR_STEP_DETECTOR:
                    if (rcvMsg->msg.msgSensorEnable.enabled) {
                        if (_stepDetectorHandle == NULL) {
                            ...
                            ASF_assert_msg(
                                OSP_STATUS_OK == OSP_Status,
                                "SensorManager: OSP_SubscribeResult (SENSOR_STEP_DETECTOR) Failed");
                        }
                    } else {
                        if (_stepDetectorHandle != NULL) {
                ...
                case CONTEXT_DEVICE_MOTION_SIGNIFICANT_MOTION:
                    if (rcvMsg->msg.msgSensorEnable.enabled) {
                        if (_sigMotionHandle == NULL) {
                            ...
                            ASF_assert_msg(
                                OSP_STATUS_OK == OSP_Status,
                                "SensorManager: OSP_SubscribeResult (SENSOR_CONTEXT_DEVICE_MOTION) Failed");
                        }
                    } else {
                        if (_sigMotionHandle != NULL) {
