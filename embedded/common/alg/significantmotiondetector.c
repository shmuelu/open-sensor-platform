...
//comes out in counts
#define NUM_COUNTS_MOTION_FOR_SIGNIFICANT_DECISION ((uint8_t)(TIME_FOR_SIGNIFICANT_DECISION / \
            TOFLT_TIME(SIGNIFICANT_MOTION_EXPECTED_SAMPLE_PERIOD)))

//filter delays for significant motion
#define SIGNIFICANT_MOTION_DETECTOR_DELAY (((MOVING_WINDOW_MEAN_BUF_SIZE - \
                1) * SIGNIFICANT_MOTION_EXPECTED_SAMPLE_PERIOD) >> 1)
 ...
 *
 ***************************************************************************************************/
void SignificantMotDetector_SetFilteredAccelerometerMeasurement(const NTTIME tstamp,
    const osp_float_t acc[NUM_ACCEL_AXES])
{
