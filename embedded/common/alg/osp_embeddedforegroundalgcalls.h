 ...
 */
#if !defined (_OSPEMBEDDEDFOREGROUNDALGCALLS_H_)
#define   _OSPEMBEDDEDFOREGROUNDALGCALLS_H_
 ...
 * -) OSPForegroundAlg_RegisterXXCallback(); (for all XX results desired)
 * -) OSPForegroundAlg_ResetAlgorithms();
 *
 * On new accel data, call:
 * -) OSPForegroundAlg_SetAccelerometerMeasurement(time, acc);
 *
 * On shut-down, call:
 * -) OSPForegroundAlg_DestroyAlgorithms();
 *
 */
...
//! Sets background result data computed from background algorithms
/*!
 *  Passes background algorithms results into foreground algorithms.
 *  This function should be called from the foreground on every callback
 *  from the background algorithms.
 *
 *  \param resultType IN integer indicating type of background result
 *  \param time IN timestamp of background result
...
/*!
* Sets the callback for calibrated sensor data.
*
*  \param fpCallback IN function pointer for calibrated sensor data callback.
*/
...
#endif //_OSPEMBEDDEDFOREGROUNDALGCALLS_H_
/*-------------------------------------------------------------------------------------------------*\
 |    E N D   O F   F I L E
\*-------------------------------------------------------------------------------------------------*/
