 ...
 *          This helper function sends Sensor Enabled indication to Sensor Acq task.
 *
 ***************************************************************************************************/
 ...
 *
 ***************************************************************************************************/
static void controlSensorEnable(uint8_t sensorId, uint8_t enable, uint16_t subResultMask)
{
    uint8_t update = 0;
    uint64_t sensorEnable = OSP_GetSubscribedResults();
        ...
        if (enable) {
            if (!(sensorEnable & (1LL << sensorId))) {
                ...
                if (sensorId == SENSOR_CONTEXT_DEVICE_MOTION) {
                    uint16_t contextDeviceMotionMask = OSP_GetSubscribedContextDeviceMotionSubResults();
 ...
 *          This helper function sends Sensor Delay setting indication to Sensor Acq task
 *
 ***************************************************************************************************/
static void SendSensorDelayIndication( uint8_t sensorId, uint16_t delayMiliSec)
{
    MessageBuffer *pSendMsg = NULLP;
...
static void controlSensorDelay(uint8_t sensorId, uint16_t miliSecondsDelay)
{
    if (sensorId < SENSOR_ENUM_COUNT) {
        if (miliSecondsDelay != sensorDelay[sensorId]) {
            ...
            SendSensorDelayIndication( sensorId,  miliSecondsDelay);
        }
    }
}
 ...
 *
 ***************************************************************************************************/
static uint16_t getSensorDelay(uint8_t sensorId)
{
    if (sensorId < SENSOR_ENUM_COUNT) {
...
/****************************************************************************************************
 * @fn      isSensorEnable
 *          Checks and returns current sensor state
 *
 ***************************************************************************************************/
static uint8_t isSensorEnable(uint8_t sensorId)
{
    uint64_t sensorEnable = OSP_GetSubscribedResults();
...
void init_android_broadcast_buffers(void)
{
    ...
    memset(broadcastPacketNextTransmittedIndex, 0, sizeof(broadcastPacketNextTransmittedIndex));        // nothing committed in any buffer
 ...
 *
 ***************************************************************************************************/
uint8_t post_on_broadcast_buffer(uint8_t *buffer, uint16_t length, struct Timestamp40_t *timeStamp)
{
    uint8_t err = 0;
        ...
        case OSP_HOST_SENSOR_GET_ENABLE:
            {
                ...
                if (command->sensorId == SENSOR_CONTEXT_DEVICE_MOTION) {
                    SlaveRegMap.enable.subResultMask = OSP_GetSubscribedContextDeviceMotionSubResults();
                } else {
